...
| 中文情境 | Karpathy框架 |
|---------|------------|
| 「Vibe coding」在中国的争议 | 用LLM写代码是新型识字能力。争议本身说明一个问题：不能从头重建=依赖而不是理解 |
| AI培训课「两天速成」 | 会用≠理解。能调用API不等于懂神经网络。问：你能写出一个最小训练循环吗？ |
| 大厂AI团队堆GPU/显卡 | 规模≠理解。先把基础弄清楚，再谈规模。Zero to Hero精神：从头实现，不是调用封装 |
| 国内「卷论文」文化 | Engineering realism：论文效果≠部署效果。「能跑通实验」和「能在生产环境稳定」是两回事 |
| 中文LLM的训练数据问题 | 正确的问题：数据质量>数据量。这是Software 3.0时代的核心挑战，不是用量能补质 |
| 互联网公司AI转型 | Vibe coding不等于随意。用LLM加速的前提是：使用者能判断输出质量，这需要底层理解 |
